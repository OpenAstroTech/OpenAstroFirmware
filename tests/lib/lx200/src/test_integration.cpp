...
ZTEST(lx200, test_slew_to_target_sequence)
{
    ...
    // Parse RA parameter
    RACoordinate ra;
    auto ra_result = parse_ra_coordinate(
        std::string(cmd1->parameters).c_str(),
        PrecisionMode::High,
        ra
    );
    zassert_ok(ra_result, "Should parse RA parameters");
    ...
    // Parse DEC parameter
    DECCoordinate dec;
    auto dec_result = parse_dec_coordinate(
        std::string(cmd2->parameters).c_str(),
        PrecisionMode::High,
        dec
    );
    zassert_ok(dec_result, "Should parse DEC parameters");
...
ZTEST(lx200, test_precision_mode_integration)
{
    ...
    RACoordinate ra;
    auto result = parse_ra_coordinate(
        std::string(cmd2->parameters).c_str(),
        PrecisionMode::Low,
        ra
    );
    zassert_ok(result, "Should parse low precision RA");
}
...
ZTEST(lx200, test_site_configuration_sequence)
{
    ...
    auto lat_result = parse_latitude_coordinate(
        std::string(cmd1->parameters).c_str(),
        lat
    );
    zassert_ok(lat_result, "Should parse latitude");
    ...
    auto lon_result = parse_longitude_coordinate(
        std::string(cmd2->parameters).c_str(),
        lon
    );
    zassert_ok(lon_result, "Should parse longitude");
    ...
    TimeValue time;
    auto time_result = parse_time_value(
        std::string(cmd3->parameters).c_str(),
        time
    );
    zassert_ok(time_result, "Should parse time");
    ...
    DateValue date;
    auto date_result = parse_date_value(
        std::string(cmd4->parameters).c_str(),
        date
    );
    zassert_ok(date_result, "Should parse date");
...
ZTEST(lx200, test_error_recovery)
{
    ...
    zassert_equal(result1, ParseResult::ErrorInvalidFormat,
                  "Should detect error");
    
    // Reset and try valid command
    parser.reset();
    for (const char c : std::string_view(":GR#")) {
        zassert_ok(parser.feed_character(c));
    }
    
    auto cmd = parser.get_command();
    zassert_true(cmd.has_value(), 
                 "Should parse valid command after error recovery");
}
...
ZTEST(lx200, test_end_to_end_performance)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        RACoordinate ra;
        parse_ra_coordinate(
            std::string(cmd->parameters).c_str(),
            PrecisionMode::High,
            ra
        );
    }
    ...
    zassert_true(avg_cycles < 20000,
                 "End-to-end parsing should be fast (<10ms requirement)");
}
